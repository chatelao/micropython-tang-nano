/*
 * APB2 Wrapper for Tiny Tapeout (TT) on Tang Nano 4K with External Pin Exposure
 *
 * Register Map (Base: 0x40002400):
 *   0x00: DATA    (W: ui_in, R: uo_out)
 *   0x04: UIO_DATA (W: uio_in_reg, R: uio_in_pin)
...
module tt_m3_wrapper (
    ...
    // Wires from TT module
    wire [7:0] uo_out;
    wire [7:0] uio_out;
    wire [7:0] uio_oe;

    // Connect external mirror ports
    assign tt_ui_in   = ui_in;
    assign tt_uo_out  = uo_out;
    assign tt_uio_out = uio_out;
    assign tt_uio_oe  = uio_oe;
    assign tt_ena     = ctrl[2];
    assign tt_clk     = ctrl[0];
    assign tt_rst_n   = ctrl[1];
    ...
    always @(*) begin
        case (PADDR[3:0])
            ...
            4'h4:    PRDATA = {24'h0, tt_uio_in}; // Read actual pin state
