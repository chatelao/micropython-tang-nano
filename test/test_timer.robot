...
Should Run Timer Test
    ...
    # Using a loop with a generous timeout to handle simulation jitter
    FOR    ${INDEX}    IN RANGE    3
        Wait For Line On Uart   Timer periodic tick    timeout=30
    END

    Wait For Line On Uart   Deinitializing timer...
    Wait For Line On Uart   Testing one-shot timer (2000 ms)...

    Wait For Line On Uart   One-shot timer fired!    timeout=30
